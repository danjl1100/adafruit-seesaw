...
macro_rules! seesaw_device {
    (
        ...
        impl_seesaw_device! {
            seesaw_parent: $crate::SeesawDevice,
            driver: $crate::Driver,
            name: $name,
        ...
        impl_seesaw_device! {
            seesaw_parent: $crate::SeesawDeviceAsync,
            driver: $crate::DriverAsync,
            name: $name,
            hardware_id: $hardware_id,
            product_id: $product_id,
            default_addr: $default_addr,
...
#[doc(hidden)]
#[macro_export(local_inner_macros)]
macro_rules! impl_seesaw_device {
    (
        seesaw_parent: $seesaw_device_name:path,
        driver: $driver:path,
        name: $name:ident,
        hardware_id: $hardware_id:expr,
        product_id: $product_id:expr,
        ...
        impl<D: $driver> $seesaw_device_name for $name<D> {
            type Driver = D;
...
macro_rules! impl_device_module {
    ($device:ident, AdcModule $({})?) => {
        ...
        impl<D: $crate::DriverAsync> $crate::modules::adc::AdcModuleAsync<D> for $device<D> {}
    };
    ...
    ($device:ident, NeopixelModule { num_leds: $num_leds:expr, pin: $pin:expr }) => {
        ...
        impl<D: $crate::DriverAsync> $crate::modules::neopixel::NeopixelModuleAsync<D>
            for $device<D>
        {
            const N_LEDS: u16 = $num_leds;
            const PIN: u8 = $pin;
    ...
    ($device:ident, StatusModule $({})?) => {
        ...
        impl<D: $crate::DriverAsync> $crate::modules::StatusModuleAsync<D> for $device<D> {}
    };
    ($device:ident, TimerModule $({})?) => {
        impl<D: $crate::Driver> $crate::modules::timer::TimerModule<D> for $device<D> {}
        impl<D: $crate::DriverAsync> $crate::modules::timer::TimerModuleAsync<D> for $device<D> {}
    };
}
...
macro_rules! maybe_async {
    (
        $(
            async @fn $name:ident $(<$(const $generic:ident : usize),+>)? ( $($args:tt)* ) -> $return:ty
            $(where $where_ty:ty : Sized $(,)?)?
            ;
        )+
    ) => {
        $(async fn $name $(<$(const $generic : usize),+>)?( $($args)* ) -> $return $(where $where_ty : Sized)? ;)+
    };
    (
        $(
            blocking @fn $name:ident $(<$(const $generic:ident : usize),+>)? ( $($args:tt)* ) -> $return:ty
            $(where $where_ty:ty : Sized $(,)?)?
            ;
        )+
    ) => {
        $(fn $name $(<$(const $generic : usize),+>)? ( $($args)* ) -> $return $(where $where_ty : Sized)? ;)+
    };
    (
        $(
            async @fn $name:ident $(<$(const $generic:ident : usize),+>)? ( $($args:tt)* ) -> $return:ty
        ...
        $(async fn $name $(<$(const $generic : usize),+>)?( $($args)* ) -> $return $(where $where_ty : Sized)? $block)+
    };
    (
        $(
            blocking @fn $name:ident $(<$(const $generic:ident : usize),+>)? ( $($args:tt)* ) -> $return:ty
        ...
        $(fn $name $(<$(const $generic : usize),+>)? ( $($args)* ) -> $return $(where $where_ty : Sized)? $block)+
    };
